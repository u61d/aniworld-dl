...
def main():
    ...
    try:
        ...
        print(
            "Please use a parameter. Use -h for Help"
        )  # if not, tells the user to specify an argument
        quit()
    ...
    if args[1] == "-h":  # if the first user argument is "-h" call the help function
        ...
        args[1] == "-u"
    ):  # if the first user argument is "-u" call the download function
        URL = args[2]
        download(URL)
    elif (
        args[1] == "-l"
    ):  # if the first user argument is "-l" call the list_dl (list download) function
        doc = args[2]

        if (
            len(args) > 3 and args[3] == "-w"
        ):  # if the second user argument is "-w" set the max_workers to the value of the third argument
            workers = int(args[4])
        else:
            workers = 4
        ...
        URL = args[
            1
        ]  # if the first user argument is the <URL> call the download function
        download(URL)
...
def help():
    ...
    print(
        "-l <doc> opens the <doc> you specify and downloads every URL line after line"
    )
    print(
        "-w <number> sets the number of parallel workers for list downloads (default: 4)"
    )
    ...
    try:
        ...
        try:
            ...
            if "mp4" in source_json:
                ...
                print(
                    "[!] Could not find downloadable URL. The site might have changed."
                )
...
def download_file_fast(
    url, filename, referer_url=None, chunk_size=8 * 1024 * 1024, max_workers=16
):
    ...
    try:
        ...
        console.print(
            f"[cyan][*] Using {optimal_workers} workers for parallel download[/cyan]"
        )
        ...
        with Progress(
            ...
            def download_chunk(chunk_info):
                ...
                try:
                    ...
                    console.print(
                        f"[red][!] Error downloading chunk {chunk_id}: {e}[/red]"
                    )
            ...
            with ThreadPoolExecutor(max_workers=optimal_workers) as executor:
                ...
                future_to_chunk = {
                    executor.submit(download_chunk, info): info
        ...
        with session.get(url, headers=headers, stream=True, timeout=30) as r:
            ...
            with Progress(
                ...
                with open(filename, "wb") as f:
                    if total_size == 0:
                        console.print(
                            "[yellow][!] Unknown file size. Downloading...[/yellow]"
                        )
                        ...
                        console.print(
                            f"[cyan][*] File size: {total_size / 1024 / 1024:.2f} MB[/cyan]"
                        )
                        task_id = progress.add_task("Downloading", total=total_size)
                        downloaded = 0
                ...
                if progress.tasks[0].started:
                    ...
                    average_speed = (
                        (final_size / total_time) / (1024 * 1024)
                        if total_time > 0
                        else 0
                    )
                    ...
                    console.print(
                        f"[green][+] Average speed: {average_speed:.2f} MB/s[/green]"
                    )
